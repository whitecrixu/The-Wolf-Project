...
# Function to configure The Wolf Project
setup_wolf() {
	if [ -d "build" ]; then
		cd build
	else
		mkdir -p build && cd build
		info "The Wolf Project has already been configured, skipping this step..."
	fi
}

# Function to build The Wolf Project
build_wolf() {
	info "Configuring The Wolf Project..."
...
move_executable() {
	local executable_name="wolf"
	cd ..
	if [ -e "$executable_name" ]; then
		info "Saving old build"
		mv ./"$executable_name" ./"$executable_name".old
	fi
	info "Moving the generated executable to the wolf folder directory..."
