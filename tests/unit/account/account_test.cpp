...
suite<"account"> accountTest = [] {
	...
	test("Account::Account default constructors") = [] {
		...
		shared_ptr<Account> byDescriptor = make_shared<Account>("wolf@test.com");

		expect(eq(byId->getID(), 1));
		expect(eq(byDescriptor->getID(), 0));

		expect(byId->getDescriptor().empty());
		expect(eq(byDescriptor->getDescriptor(), string{"wolf@test.com"}));
	...
	vector<AccountLoadTestCase> accountLoadTestCases{
		...
		{"returns by descriptor if exists", make_shared<Account>("wolf@test.com"), AccountErrors_t::Ok},
	...
	for (auto& testCase : accountLoadTestCases) {
		test(testCase.description) = [&injectionFixture, &testCase] {
			...
			accountRepository.addAccount("wolf@test.com", AccountInfo{1, 1, 1, AccountType::ACCOUNT_TYPE_GOD});
	...
	test("Account::reload reloads account info") = [&injectionFixture] {
		...
		accountRepository.addAccount("wolf2@test.com", AccountInfo { 1, 1, 1, AccountType::ACCOUNT_TYPE_GAMEMASTER });
	...
	test("Account::getCoins returns coins for specified account only") = [&injectionFixture] {
		...
		accountRepository.addAccount("wolf2@test.com", AccountInfo { 2, 1, 1, AccountType::ACCOUNT_TYPE_GOD });
	...
	test("Account::getAccountPlayer returns players") = [&injectionFixture] {
		...
		accountRepository.addAccount(
			"wolf@test.com",
			AccountInfo { 1, 1, 1, AccountType::ACCOUNT_TYPE_GOD, {{ "Wolf", 1 }, { "Wolf2", 2 }} }
 		);
		...
		expect(eq(players["Wolf"], 1));
		expect(eq(players["Wolf2"], 2));
	};
	...
	test("Account::authenticate using sessions") = [&injectionFixture] {
		...
		accountRepository.addAccount(
			"session-key",
			AccountInfo { 1, 1, 1, AccountType::ACCOUNT_TYPE_GOD, { { "Wolf", 1 }, { "Wolf2", 2 } }, false, getTimeNow() + 24 * 60 * 60 * 1000 }
		);
		);

		const auto hasCharacter = accountRepository.getCharacterByAccountIdAndName(1, "Wolf");
