-- Avalanche
-- Auto-converted from XML

local monster = Game.createMonsterType("Avalanche")
if not monster then return end

monster:name("Avalanche")
monster:nameDescription("avalanche")
monster:health(550)
monster:maxHealth(550)
monster:experience(305)
monster:speed(195)
monster:race(RACE_UNDEAD)
monster:corpseId(7349)
monster:outfit({lookType = 261})
monster:defense(27)
monster:armor(26)

-- Flags
monster:attackable(true)
monster:hostile(true)
monster:pushable(false)
monster:canPushItems(true)

-- Resistances
monster:elements({
    {type = COMBAT_FIREDAMAGE, percent = 30},
    {type = COMBAT_ENERGYDAMAGE, percent = -10},
})

-- Immunities
monster:immunities({
    {condition = CONDITION_FREEZING, immunity = true},
    {condition = CONDITION_POISON, immunity = true},
    {condition = CONDITION_OUTFIT, immunity = true},
    {condition = CONDITION_DRUNK, immunity = true},
    {condition = CONDITION_INVISIBLE, immunity = true},
})

-- Voices
monster:voices({
    {text = "You will pay for imprisoning me here.", yell = false},
    {text = "Puny warmblood.", yell = false},
})

-- Attacks (for reference, implement with spell system)
--[[
-- name=melee, interval=2000, chance=100, skill=50, attack=60
-- name=drown, interval=1000, chance=100, min=-10, max=-50, length=5, spread=6, areaEffect=energyarea
-- name=speed, interval=4000, chance=100, radius=3, duration=10000, speedchange=-400, areaEffect=poff
-- name=physical, interval=6000, chance=100, range=5, max=-40, radius=1, target=1, shootEffect=largerock
--]]

monster:register()
