...
local function checkAndLogDuplicateValues(tableNames)
	for _, tableName in ipairs(tableNames) do
		...
		if type(varTable) == "table" then
			...
			if duplicates then
				print("  - [WARNING] Checking " .. tableName .. ": Duplicate values found: " .. table.concat(duplicates, ", "))
			else
				print("  - Checking " .. tableName .. ": No duplicate values found.")
			end
		else
			print("  - [WARNING] " .. tableName .. " is not a table. Unable to check for duplicate values.")
		end
