-- Boss Events (RevScriptSys)
-- Handles reward boss participation tracking and loot distribution
...
-- Boss Death Event
local function bossDeathCallback(creature, corpse, killer, mostDamageKiller, lastHitUnjustified, mostDamageUnjustified)
...
bossDeath:onDeath(bossDeathCallback)
bossDeath:register()

-- Boss Participation Event (tracks players in boss fight)
local function bossParticipationCallback(creature, interval)
	local bossId = creature:getId()
	local info = globalBosses[bossId]
...
bossParticipation:type("think")
bossParticipation:onThink(bossParticipationCallback)
bossParticipation:register()

-- Boss Health Change Event (tracks damage and healing)
local function bossHealthChangeCallback(creature, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType, origin)
...
local bossHealthChange = CreatureEvent("BossHealthChange")
bossHealthChange:type("healthchange")
bossHealthChange:onHealthChange(bossHealthChangeCallback)
bossHealthChange:register()

-- Boss Appear Event (initializes reward boss)
local function bossAppearCallback(creature)
	if creature:getType():isRewardBoss() then
		creature:setReward(true)
	end
end

-- Note: onCreatureAppear is not available in CreatureEvent for creatures
-- This needs to be handled differently (possibly via MonsterType callbacks or spawn scripts)
