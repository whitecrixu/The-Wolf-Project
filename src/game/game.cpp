...
void Game::loadBoostedCreature() {
	...
	if (date == ltm->tm_mday) {
		...
		std::cout << "  - \033[1;36mBoosted creature: " << result->getString("boostname") << "\033[0m" << std::endl;
		return;
	}
	...
	std::cout << "  - \033[1;36mBoosted creature: " << selectedMonster.name << "\033[0m" << std::endl;
...
FILELOADER_ERRORS Game::loadAppearanceProtobuf(const std::string &file) {
	using namespace Wolf::protobuf::appearances;
...
void Game::logCyclopediaStats() {
	// Log monsters count
	size_t monstersCount = g_monsters().monsters.size();
	g_logger().info("Loaded {} monsters", monstersCount);
}
