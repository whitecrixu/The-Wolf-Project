...
void Connection::accept()
{
	std::lock_guard<std::recursive_mutex> lockClass(connectionLock);
	try {
		readTimer.expires_after(std::chrono::seconds(Connection::read_timeout));
...
void Connection::internalSend(const OutputMessage_ptr& msg)
{
	...
	try {
		writeTimer.expires_after(std::chrono::seconds(Connection::write_timeout));
...
void Connection::dispatchBroadcastMessage(const OutputMessage_ptr& msg)
{
	...
	msgCopy->append(msg);
	boost::asio::dispatch(socket.get_executor(), std::bind(&Connection::broadcastMessage, shared_from_this(), msgCopy));
}
