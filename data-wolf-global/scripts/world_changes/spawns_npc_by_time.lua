		...
		addEvent(function()
			for _, entry in ipairs(npcsToAdd) do
				...
				if npc then
					...
					print("  - [ERROR] NpcByTime: Failed to spawn: " .. entry.data.name)
				end
			end
		end, 5000)
	end
...
function spawnsNpcBySpawn.onStartup()
	...
	if #npcsToAdd > 0 then
		addEvent(function()
			for _, entry in ipairs(npcsToAdd) do
				...
				if npc then
					...
					npc:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
					activeNpcRefs[entry.index] = npc:getId()
					print("  - NpcByTime: " .. entry.data.name .. " spawned successfully.")
				else
					print("  - [ERROR] NpcByTime: Failed to spawn: " .. entry.data.name)
				end
			end
		...
		print("  - NpcByTime: No NPCs to spawn in the current period.")
	end
