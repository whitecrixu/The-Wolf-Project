...
OT_DB_USER="${OT_DB_USER:-wolf}"
OT_DB_PASSWORD="${OT_DB_PASSWORD:-wolf}"
OT_DB_DATABASE="${OT_DB_DATABASE:-wolf}"
...
OT_SERVER_DATA="${OT_SERVER_DATA:-data-wolf-global}"
OT_SERVER_MAP="${OT_SERVER_MAP:-https://wolf-project.org/releases/download/v3.1.0/wolf.otbm}"
...
if [ "$OT_SERVER_DATA" = "data-wolf-global" ] && [ ! -f data-wolf-global/world/wolf.otbm ]; then
	echo "YES"

	echo "Downloading OTBR Map..."
	wget --no-check-certificate "$OT_SERVER_MAP" -O data-wolf-global/world/wolf.otbm
...
if [[ $(mysql -u "$OT_DB_USER" -p"$OT_DB_PASSWORD" -h "$OT_DB_HOST" -e 'SHOW TABLES LIKE "server_config"' -D "$OT_DB_DATABASE") ]]; then
	...
	echo "Import Wolf-Server Schema"
	...
	if [ "$OT_SERVER_TEST_ACCOUNTS" = "true" ]; then
		...
		mysql -u "$OT_DB_USER" -p"$OT_DB_PASSWORD" -h "$OT_DB_HOST" --port="$OT_DB_PORT" -D "$OT_DB_DATABASE" </wolf/01-test_account.sql
		mysql -u "$OT_DB_USER" -p"$OT_DB_PASSWORD" -h "$OT_DB_HOST" --port="$OT_DB_PORT" -D "$OT_DB_DATABASE" </wolf/02-test_account_players.sql
	else
